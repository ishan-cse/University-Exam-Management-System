...
<div class="card">
  <div class="card-header">
    <b> Assign Supplementary Exam to Student </b>
  </div>
  <div class="card-body">
    <blockquote class="blockquote mb-0">


      <form method="post" action="{{ url('/course_coordinator/assign/supplemetary_exam/final_step_process') }}" enctype="multipart/form-data">
          @csrf
            ...
            <div class="col-sm-9">
              <select id="exampleFormControlSelect1" class="form-control" name="batch_course_list_id" required>
                ...
                @php
                  $mid=NULL;
                  $final=NULL;
                if($student_assigned_course->mid_term_marks_attend_status==0){
                  $mid=$student_assigned_course->mid_term_marks;
                }
                if($student_assigned_course->final_marks_attend_status==0){
                  $final=$student_assigned_course->final_marks;
                }
                $seventy_marks=$mid+$final;
                @endphp

                @if($seventy_marks>=15 && $seventy_marks<=27)
                    @php
                      $course_title=DB::table('batch_course_lists')->where('department_id','=',$student_assigned_course->department_id)->where('batch_id','=',$student_assigned_course->batch_id)->where('course_version_id','=',$student_assigned_course->course_version_id)->where('id','=',$student_assigned_course->batch_course_list_id)->value('course_title');
                    @endphp
