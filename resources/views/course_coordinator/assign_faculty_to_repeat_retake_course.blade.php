...
<div class="card">
  <div class="card-header">
    <b> Assign Faculty to Repeat/Retake (without the Batch) Course </b>
  </div>
  <div class="card-body">
    <blockquote class="blockquote mb-0">


      <form method="post" action="{{ url('course_coordinator/assign_faculty_to_repeat_retake_course_process') }}" enctype="multipart/form-data">
          @csrf
          ...
          <div class="form-group row">
            ...
            <div class="col-sm-9">
              <input type="text" class="form-control @error('student_id') is-invalid @enderror" id="student_id" name="student_id" value="{{$faculty_assign_course_details->student_id}}" required readonly>
          ...
          @php
            //$batch_name=DB::table('batches')->where('id','=',$faculty_assign_course_details->batch_id)->value('batch_name');
            $course_details=DB::table('batch_course_lists')->where('department_id','=',$faculty_assign_course_details->department_id)->where('id','=',$faculty_assign_course_details->batch_course_list_id)->first();
          @endphp
